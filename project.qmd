...
```{r}
print(colnames(WB_wide))
print(colnames(FAO_widetest))

```

```{r}
print(sapply(WB_wide, class))
print(sapply(FAO_widetest, class))
...
```{r}
setdiff(WB_wide$Year, FAO_widetest$Year)
setdiff(WB_wide$Area, FAO_widetest$Area)
...
# Identifier l'année avec le moins de données manquantes
year_least_missing <- which.min(apply(data_plot[, c("GDP growth (annual %)", "Prevalence of low birthweight (percent)")], 2, function(x) sum(is.na(x))))
year_least_missing <- unique(data_plot$Year[year_least_missing])

# Filtrer les données pour cette année et enlever les valeurs manquantes
data_plot <- na.omit(data_plot[data_plot$Year == year_least_missing, c("Year", "Area", "GDP growth (annual %)", "Prevalence of low birthweight (percent)")])
...
# Afficher le résumé statistique des deux variables
...
```{r}
# Créer une copie du dataframe original pour l'année 2020
data_plot_2020 <- data

# Filtrer les données pour l'année 2020 et enlever les valeurs manquantes
...
# Créer des catégories basées sur le taux de prévalence
...
# Graphique de dispersion avec couleurs par catégorie de prévalence
...
# Sous-ensemble des données avec les colonnes sélectionnées
selected_data1 <- data[, selected_cols1]

# Filtrer les lignes avec des valeurs manquantes pour l'indicateur spécifié
...
# Identifier l'année avec le moins de valeurs manquantes
year_with_least_na <- which.min(table(filtered_data1$Year))

# Sélection des données pour cette année
...
```{r}
# Filtrer les lignes avec des valeurs non manquantes pour les deux variables
...
# Identifier l'année avec le deuxième moins de valeurs manquantes
second_year_with_least_na <- which.min(table(filtered_data_non_na$Year)[-year_with_least_na])

# Sélection des données pour cette deuxième année
...
```{r}
# Assuming you have a data frame named 'data' with relevant columns

# Select relevant columns for the chosen year (2013)
selected_year <- 2013
...
# Select relevant columns for the chosen year (2018)
year_2018 <- 2018
...
# Calculate the correlation coefficient for 2018
...
# Print the correlation coefficient for 2018
...
# Supposons que votre dataframe s'appelle 'data_countries_PIB'
# Assurez-vous que les colonnes Year, Area, et Prevalence of anemia among women of reproductive age (15-49 years) existent

# Créer le graphique de l'évolution de l'indicateur d'anémie par pays
...
```{r}
# Exemple pour exclure les lignes avec des valeurs manquantes pour les variables d'intérêt
...
# Calculer la corrélation avec les données complètes
...
# Ajustez le modèle linéaire avec les trois variables explicatives
...
```{r}
# Créer la formule avec tous les indicateurs du World Bank
formula_all <- formula(`Prevalence of undernourishment (percent) (3-year average)` ~ 
                        `GDP growth (annual %)` + 
...
# Ajuster le modèle linéaire avec toutes les variables explicatives
...
# Encadrement des noms de variables entre backticks
selected_indicators <- paste("`", selected_indicators, "`", sep = "")

# Création de la formule avec les indicateurs sélectionnés
...
# Ajustement du modèle linéaire avec les indicateurs sélectionnés
...
First of all, we decided add all the World Bank indicators such as: GDP growth (annual %), GDP per capita growth (annual %), Agricultural land (% of land area), Children in employment, female (% of female children ages 7-14), Children in employment, male (% of male children ages 7-14), Children out of school (% of primary school age), Children out of school, female (% of female primary school age), Children out of school, male (% of male primary school age), Death rate, crude (per 1,000 people), Current education expenditure, total (% of total expenditure in public institutions), Current health expenditure (% of GDP), Food production index (2014-2016 = 100), Gini index, Labor force participation rate, female (% of female population ages 15+) (modeled ILO estimate), Labor force, female (% of total labor force), Life expectancy at birth, total (years), Lifetime risk of maternal death (%) and the Number of infant deaths.

We did it just to have a brief look at all the indicators and to compare them each to other to see which are more significant in out case.

After that, we chose the indicators which were significant: GDP growth (annual %), Agricultural land (% of land area), Current education expenditure, total (% of total expenditure in public institutions) and Life expectancy at birth, total (years)".
